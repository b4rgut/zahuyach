...
ZahuyachApp.ui = {
  ...
  showTooltip(element, text) {
    ...
    tooltip.className =
      "fixed z-50 px-2 py-1 text-xs bg-gh-canvas-overlay border border-gh-border-default rounded shadow-lg pointer-events-none";
  ...
  saveBookmarks(bookmarks) {
    try {
      ...
      document.dispatchEvent(
        new CustomEvent("zahuyach:bookmarksChanged", {
          detail: { bookmarks },
        }),
      );
    } catch (error) {
  ...
  loadExpandedCategories() {
    ...
    expanded.forEach((slug) => {
      ...
      const chevron = document.querySelector(
        `[data-category="${slug}"] .category-chevron`,
      );
...
ZahuyachApp.utils = {
  ...
  setupShareButtons() {
    document.addEventListener("click", (e) => {
      if (
        e.target.matches("[data-share]") ||
        e.target.closest("[data-share]")
      ) {
        e.preventDefault();
        const button = e.target.matches("[data-share]")
          ? e.target
          : e.target.closest("[data-share]");
  ...
  setupCopyButtons() {
    document.addEventListener("click", (e) => {
      if (e.target.matches("[data-copy]") || e.target.closest("[data-copy]")) {
        e.preventDefault();
        const button = e.target.matches("[data-copy]")
          ? e.target
  ...
  async copyToClipboard(text) {
    try {
      ...
      try {
        ...
        ZahuyachApp.ui.showNotification(
